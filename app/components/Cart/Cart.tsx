  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center transition-opacity duration-300 ease-in-out">
      <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto transition-transform transform duration-300 ease-in-out">
        ...
        {cartItems.length === 0 ? (
          <p className="text-black-600">Seu carrinho está vazio</p>
        ) : (
          <>
        ...
        <div className="mt-6 flex justify-between">
          <button
            onClick={onClose}
            className="bg-purple-500 text-black-700 hover:bg-gray-300 py-2 px-4 rounded-lg w-full"
          >
            Continuar Comprando
          </button>
          {cartItems.length === 0 ? (
            <button
              className="bg-green-500 text-black-700 hover:bg-gray-300 py-2 px-4 rounded-lg w-full ml-4"
              disabled
            >
              Finalizar Compra
            </button>
            ...
            <button
              ...
              className="bg-green-500 text-black hover:bg-blue-600 py-2 px-4 rounded-lg w-full ml-4"
            >
              Finalizar Compra
            </button>
          )}
